...
function text_transform(text, operation) {
    switch (operation) {
        case "uppercase":
...
function list_manager(action, list_name, item) {
    if (action === "create") {
        _lists[list_name] = [];
    ...
    switch (action) {
        ...
        case "remove": {
            ...
            if (idx === -1)
                return `'${item}' not found in '${list_name}'.`;
            lst.splice(idx, 1);
...
function data_formatter(data, format) {
    if (!data.length)
    ...
    if (format === "csv") {
        ...
        for (const row of data)
            rows.push(keys.map((k) => String(row[k] ?? "")).join(","));
        return rows.join("\n");
    }
...
function datetime_info(operation) {
    const now = new Date();
    switch (operation) {
        case "now":
...
function runTool(name, input) {
    switch (name) {
        ...
        case "text_transform":
            return text_transform(input.text, input.operation);
        case "list_manager":
            if (input.action === "replace" && Array.isArray(input.items)) {
                const serialized = input.items.map((v) => String(v)).join("\n");
                return list_manager("replace", input.list_name, serialized);
            }
            return list_manager(input.action, input.list_name, input.item);
        case "data_formatter":
            return data_formatter(input.data, input.format);
        case "datetime_info":
            return datetime_info(input.operation);
        default:
