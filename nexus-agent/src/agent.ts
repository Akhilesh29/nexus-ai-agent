...
async function callGemini(userTask: string, toolContext?: string): Promise<string> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error(
      "GEMINI_API_KEY is not set. Add it to your environment to enable open-ended AI answers."
    );
  }
...
function parseTask(task: string): { tool: string; input: Record<string, unknown> } | null {
  ...
  if (lower.includes('day') || lower.includes('time') || lower.includes('date')) {
    let operation = 'now';
    if (lower.includes('day')) operation = 'day_of_week';
